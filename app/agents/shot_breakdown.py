...
class ShotBreakdownAgent:
    ...
    TARGET_SHOT_DURATION = 9.0  # point de départ par défaut, pas une limite dure
    ...
    def break_down_episode(self, script_content: str, episode_title: str,
        ...
        system_prompt = f"""
⚠️ LANGUAGE RULE — READ THIS FIRST, IT OVERRIDES EVERYTHING BELOW (including the fact that
these instructions are written in French): Your ENTIRE response MUST be written in the SAME
language as the script provided below — NOT necessarily French. If the script is in English,
respond entirely in English. Detect its actual language and mirror it exactly, regardless of
what language the rest of these instructions are written in.
...
2. Chaque plan doit viser environ {self.TARGET_SHOT_DURATION} secondes comme point de départ par défaut —
   ne descends en dessous que si le plan est vraiment court (une réaction rapide), et ne dépasse
   JAMAIS {self.MAX_SHOT_DURATION} secondes (plafond technique strict). Un plan trop long dans le script
