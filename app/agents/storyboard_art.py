...
DEFAULT_STORYBOARD_MODEL = "qwen-image-2.0-pro"

# Plafond CONFIRMÉ (message d'erreur réel) pour certains modèles Qwen-Image :
# 0-3 images de référence maximum. Pas de preuve que les autres modèles de la
# liste partagent cette limite — on ne restreint donc QUE ceux-ci, pas tous.
MODELS_WITH_3_IMAGE_CAP = {"qwen-image-2.0-pro", "qwen-image-2.0"}
MAX_REFS_FOR_CAPPED_MODELS = 3
...
"""
    ...
    def _analyze_shot_with_vision(self, shot: dict, structured_characters: list, structured_location: dict,
                                   ...
                                   forced_layout: str = None, previous_storyboard_path: str = None) -> dict:
        ...
        générée. previous_storyboard_path : planche du plan précédent, si et
        SEULEMENT SI générée en grille (une image "one_frame" n'a pas de
        "dernier panneau" identifiable — géré côté appelant). Sert de
        référence d'ÉTAT uniquement (niveau 3 de la hiérarchie APPEARANCE
        STATE), jamais de référence de cadrage. Retourne {} si aucune image
        disponible ou si l'appel échoue (repli silencieux vers
        _expand_shot_to_beats côté appelant)."""
        image_labels = []
        ...
        if previous_storyboard_path:
            image_labels.append((
                "PREVIOUS SHOT — final storyboard (state reference only, bottom-right panel = "
                "starting state for this shot; ignore its framing/composition):",
...
(closeup and fullbody), the location plate, and — if provided — the final storyboard
of the PREVIOUS shot. These images define what EXISTS. Your job is to design the
storyboard grid for ONE shot, grounded in these references.

REFERENCE GROUNDING (CRITICAL):
- The references are the single source of truth for physical identity: faces,
  hair, proportions, and the set's actual layout, depth and lighting.
- Your beats must be EXECUTABLE with what the references show. Anchor actions and
  compositions in elements actually visible in them (a garment detail, an object
  a character holds, a set element at a specific position) — never stage an action
  requiring a prop, costume piece or set element that no reference contains. If
  the shot description mentions an object absent from all references, stage it
...
APPEARANCE STATE (authority hierarchy — apply in this exact order):
1. If the shot description explicitly states a change of outfit, physical condition
   or form for a character, that description WINS over every image reference.
2. Otherwise, if an appearance state is provided as text input for this shot, it is
   authoritative.
3. Otherwise, if the PREVIOUS shot's storyboard is provided among the references,
   the character's appearance in its LAST panel (reading order: left to right, top
   to bottom — the bottom-right panel) is the state at the START of this shot.
   Maintain it exactly across all panels. (This level does not apply if no previous
   storyboard reference is provided — skip directly to level 4.)
4. Otherwise, default to the outfit shown on the character sheet.
Rules:
- When a state changed at level 1, no element of the previous state may remain
  visible in any panel, on this character or on anyone else.
- If the resolved state contradicts what the character sheet or the previous
  storyboard shows, the resolved state wins — and you MUST report the conflict in
  "continuity_note" so the downstream image prompt can enforce it.
- The previous storyboard is a state reference ONLY — never copy its framings,
  compositions or staging into your panels.
...
6. Use the previous/next shot descriptions if provided: the first panel inherits
   the situation coming from the previous shot, and the last panel hands over
   cleanly to the next — without depicting events that belong to the next shot.
7. PROGRESSIVE CONSISTENCY: panels are sequential in time — any state that evolves
   over the course of the shot (physical condition, environment, an object, an
   emotional state, anything established by APPEARANCE STATE or the shot
   description) must progress consistently forward across panels, never reverting
   to an earlier state in a later panel — even when that panel is a tight close-up
   isolating a single detail. A close-up panel must still reflect the CURRENT
   accumulated state from prior panels, not reset to the shot's starting state.
8. Respond ONLY in valid JSON, in the same language as the shot description.
...
Format: {{
  ...
  "appearance_state": "per visible character: resolved outfit and physical condition for this shot, plus what must NOT appear, 1 short sentence each",
  "continuity_note": "what must visually carry over from/to neighboring shots, including any conflict between the resolved state and a reference image, or empty string",
...
"""
    ...
    def _build_prompt(self, shot: dict, style_label: str, mode: str = "one_frame", analysis: dict = None,
        ...
        identity_lock = (
            ...
            "images provided — do not alter their face, proportions, or defining features; "
            "their outfit follows the APPEARANCE STATE below. This lock applies ONLY to the "
            "specific named characters shown in the reference images. If the scene mentions "
            "any OTHER person not among the reference images (e.g. a background character, a "
            "stranger, an unnamed passerby), invent them as a completely distinct, independent "
            "individual — never reuse the reference character's face, body, or likeness for "
            "them, even if no visual reference exists for that other person."
        )
        if grounded_appearance:
            identity_lock += f" What the reference image actually shows: {grounded_appearance}"
        parts = [identity_lock]
        if analysis and analysis.get("appearance_state"):
            parts.append(f"APPEARANCE STATE (authoritative): {analysis['appearance_state']}")
        ...
        if mode != "one_frame" and beats:
            ...
            parts.append(
                f"COMPOSITION: arrange as EXACTLY {panel_count} panels in a {layout} grid — never "
                f"more, never fewer. Do not add an establishing panel, a title panel, or any extra "
                f"panel beyond the {panel_count} specified. Each panel has a DISTINCT shot size "
                "and/or camera angle (mix wide/medium/close-up and eye-level/low/high angle across "
                "panels) — a professional storyboard grid with real cinematic variety, not repeated "
                "identical framing."
            )
            parts.append(
                "PROGRESSIVE CONSISTENCY: any state that evolves over the course of the shot "
                "(physical condition, environment, an object, an emotional state) must progress "
                "consistently forward across panels in reading order — never revert to an earlier "
                "state in a later panel, even a tight close-up isolating one detail."
            )
        else:
            parts.append(
    ...
    # ------------------------------------------------------------------
    def _prioritize_references_for_cap(self, reference_image_paths: list, structured_characters: list,
                                        structured_location: dict, max_refs: int) -> list:
        """Certains modèles (voir MODELS_WITH_3_IMAGE_CAP) refusent purement
        et simplement plus de N images de référence — confirmé par un vrai
        message d'erreur, pas une supposition. Avec plusieurs personnages
        (closeup+fullbody chacun) + décor, on dépasse vite ce plafond.
        Priorité : 1 image par personnage (fullbody de préférence — porte à
        la fois le visage ET la tenue, contrairement au closeup seul) + le
        décor si la place le permet. Compromis, pas parfait, mais évite un
        échec pur et simple."""
        if len(reference_image_paths) <= max_refs:
            return reference_image_paths

        prioritized = []
        for char in (structured_characters or []):
            # Fullbody préféré (porte visage + tenue) ; à défaut, closeup.
            chosen = char.get("fullbody_path") or char.get("closeup_path")
            if chosen:
                prioritized.append(chosen)

        loc_path = structured_location.get("path") if structured_location else None
        if loc_path and len(prioritized) < max_refs:
            prioritized.append(loc_path)

        # Si même "1 par personnage" dépasse encore le plafond (beaucoup de
        # personnages), on tronque strictement — mieux qu'un échec total.
        return prioritized[:max_refs] if prioritized else reference_image_paths[:max_refs]
    ...
    def generate_storyboard_frame(self, shot: dict, reference_image_paths: list, visual_styles: list,
                                   ...
                                   structured_location: dict = None, use_prompt_director: bool = True,
                                   previous_storyboard_path: str = None) -> dict:
        ...
        previous_storyboard_path : planche du plan précédent, UNIQUEMENT si
        elle a été générée en grille (jamais en one_frame — pas de "dernier
        panneau" identifiable dans ce cas, voir _analyze_shot_with_vision).
        Résolution à faire côté appelant. Ignoré en mode "one_frame".
        ...
        final_ref_paths = reference_image_paths
        if model in MODELS_WITH_3_IMAGE_CAP and reference_image_paths and len(reference_image_paths) > MAX_REFS_FOR_CAPPED_MODELS:
            final_ref_paths = self._prioritize_references_for_cap(
                reference_image_paths, structured_characters, structured_location, MAX_REFS_FOR_CAPPED_MODELS
            )

        if not final_ref_paths:
            result = self.router.generate_image(prompt)
        else:
            result = self.router.generate_image_edit(prompt, final_ref_paths, model=model)
