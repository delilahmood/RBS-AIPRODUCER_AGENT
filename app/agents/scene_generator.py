...
"""
    ...
    def _build_happyhorse_prompt(self, shot: dict, labels: list, style_label: str, has_dialogue: bool,
                                  action_beats: dict = None, storyboard_beats: list = None) -> str:
        duration = min(shot.get("duration_seconds", 10) or 10, 10)
        ref_note = ", ".join(labels) if labels else "the reference images"

        # Mode multi-shot : réutilise directement les panneaux du storyboard
        # en grille (même choix créatif que celui déjà fait au moment du
        # storyboard — single = mouvement continu, grid = plusieurs cadrages
        # distincts). Confirmé pour HappyHorse (documentation officielle :
        # gère nativement les coupures de caméra à l'intérieur d'un même
        # appel) — pas encore confirmé pour Wan, donc réservé à HappyHorse.
        if storyboard_beats:
            n = len(storyboard_beats)
            ...
            parts = [
                ...
                "RENDER DISCIPLINE: use the referenced storyboard for identity and story "
                "continuity — each panel below is a DISTINCT shot with its own camera cut, "
                "not a single continuous take. No panel borders, no storyboard furniture, no "
                "text, in the final output.",
            ]
            for i, beat in enumerate(storyboard_beats):
                start_t = i * seg_duration
                end_t = (i + 1) * seg_duration
                parts.append(f"Shot {i + 1} ({start_t:.1f}-{end_t:.1f}s): {beat}")
        else:
            parts = [
                f"FORMAT: {duration:.0f}s / 720P / cinematic live-action.",
                f"REFERENCES: {ref_note}.",
    ...
    def _build_prompt(self, shot: dict, labels: list, style_label: str, has_dialogue: bool, model: str,
        ...
        if model.startswith("happyhorse"):
            return self._build_happyhorse_prompt(shot, labels, style_label, has_dialogue, action_beats, storyboard_beats)
        # Mode multi-shot pas encore confirmé pour Wan -> repli sur le mode continu classique
    ...
    def generate_scene_video(self, shot: dict, storyboard_path: str, character_refs: list, location_ref: dict,
                              ...
                              custom_prompt: str = None, storyboard_beats: list = None) -> dict:
        ...
        storyboard_beats : les descriptions des 9 panneaux si le storyboard
        de ce plan a été généré en mode grille — réutilisées directement en
        mode multi-shot pour HappyHorse (même choix créatif que celui déjà
        fait au moment du storyboard). Ignoré pour Wan (non confirmé).
        ...
        use_multishot = bool(storyboard_beats) and model.startswith("happyhorse")

        if custom_prompt:
            prompt = custom_prompt
        else:
            character_names = [c["name"] for c in (character_refs or [])]
            # Pas besoin de l'enrichissement narratif séparé si on réutilise
            # déjà les beats du storyboard (économise un appel texte).
            action_beats = None if use_multishot else self._generate_action_beats(shot, character_names, has_dialogue)
            prompt = self._build_prompt(
                shot, labels, style_label, has_dialogue, model, action_beats,
                storyboard_beats=storyboard_beats if use_multishot else None
            )
